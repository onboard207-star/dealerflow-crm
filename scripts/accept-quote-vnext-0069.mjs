...
const run = (command,args,env) => new Promise((resolve,reject)=>{const child=spawn(command,args,{env,stdio:["ignore","pipe","pipe"]});let out="",err="";child.stdout.on("data",d=>out+=d);child.stderr.on("data",d=>err+=d);child.on("exit",code=>code===0?resolve(out):reject(new Error(err||out||`${command} failed ${code}`)));});
...
try {
  ...
  await run("node",["scripts/seed-synthetic-pilot.mjs","--confirm","SYNTHETIC-DEMO"],{...process.env,DATABASE_URL:dbUrl.toString(),APP_ENV:"test"});
