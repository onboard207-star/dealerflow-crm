...
try {
  ...
  try {
    ...
    await q(client,`INSERT INTO quote_product_scenarios(id,organization_id,quote_id,stable_key,position,product_kind,product_reference,label,source_type) VALUES('qps_acceptance_draft',$1,'quo_acceptance_1','draft:acceptance',0,'vehicle','veh_quote_1','Synthetic draft product','application')`,[org]);
    ...
    const immutableDelete=await expectFailure(client,"immutable delete",`DELETE FROM quote_product_scenarios WHERE organization_id=$1 AND id=$2`,[org,immutableScenario.id]);
