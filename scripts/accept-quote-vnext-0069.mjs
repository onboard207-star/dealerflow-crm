...
try {
  ...
  try {
    ...
    for(let i=0;i<6;i++){
      ...
      await q(client,`INSERT INTO deal_quotes(id,organization_id,deal_id,version,status,purchase_type,subtotal_cents,fee_cents,tax_cents,discount_cents,total_cents,presented_at,accepted_at,idempotency_key,created_by,updated_by) VALUES($1,$2,$3,1,$4::quote_status,$5,3000000,100000,200000,-100000,3200000,CASE WHEN $4::text IN ('presented','accepted') THEN now() END,CASE WHEN $4::text='accepted' THEN now() END,$6,$7,$7)`,[id,org,base[i].deal_id,status,purchase,`acceptance:${i+1}`,base[i].owner_user_id]);
    ...
    await q(client,`INSERT INTO incentive_programs(id,organization_id,code,name,source_type,source_label) VALUES('inc_acceptance_program',$1,'SYN-1000','Synthetic acceptance incentive','dealer-program','Acceptance fixture')`,[org]);
    ...
    await q(client,"DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname='quote_vnext_acceptance_runtime') THEN CREATE ROLE quote_vnext_acceptance_runtime NOLOGIN; END IF; END $$"); await q(client,"GRANT quote_vnext_acceptance_runtime TO CURRENT_USER"); await q(client,"GRANT USAGE ON SCHEMA public TO quote_vnext_acceptance_runtime"); await q(client,"GRANT SELECT ON deal_quotes TO quote_vnext_acceptance_runtime"); await q(client,"GRANT SELECT,INSERT,UPDATE,DELETE ON quote_product_scenarios,quote_payment_scenarios,quote_trade_snapshots TO quote_vnext_acceptance_runtime");
    ...
    const authorizedCreation=(await q(client,`INSERT INTO quote_product_scenarios(id,organization_id,quote_id,stable_key,position,product_kind,product_reference,label,source_type) VALUES('qps_acceptance_draft',$1,'quo_acceptance_1','draft:acceptance',1,'vehicle','veh_quote_1','Synthetic draft product','application') RETURNING id`,[org])).rowCount===1;
    ...
    const immutableUpdate=await expectDeniedMutation(client,"immutable update",`UPDATE quote_product_scenarios SET label='changed' WHERE organization_id=$1 AND id=$2 RETURNING id`,[org,immutableScenario.id]);
    const immutableDelete=await expectDeniedMutation(client,"immutable delete",`DELETE FROM quote_product_scenarios WHERE organization_id=$1 AND id=$2 RETURNING id`,[org,immutableScenario.id]);
    const incompleteLease=await expectFailure(client,"incomplete lease",`INSERT INTO quote_payment_scenarios(id,organization_id,quote_id,product_scenario_id,stable_key,position,mode,calculation_status,cash_down_cents,mode_metadata,source_type,source_label,calculation_policy_version,rounding_policy_version,calculation_fingerprint) VALUES('qpy_incomplete_lease',$1,$2,$3,'lease:bad',8,'lease','incomplete',0,'{}','manual-entry','Synthetic invalid lease','test','test',repeat('a',64))`,[org,scenario.quote_id,scenario.id]);
    ...
    for(let i=0;i<2;i++) await q(client,`INSERT INTO trade_appraisals(id,organization_id,deal_id,vehicle_id,version,status,allowance_cents,payoff_cents,equity_cents,idempotency_key) VALUES($1,$2,$3,$4,1,'accepted',$5,$6,$7,$8)`,[`tap_multi_${i+1}`,org,scenario.deal_id,tradeVehicles[i].id,i?900000:1500000,i?1200000:1000000,i?-300000:500000,`multi:${i}`]);
    for(let i=0;i<2;i++) await q(client,`INSERT INTO quote_trade_snapshots(id,organization_id,quote_id,product_scenario_id,trade_appraisal_id,position,appraisal_version,appraisal_revision_at,appraisal_allowance_cents,quote_allowance_cents,payoff_cents,equity_cents,quote_adjustment_cents,tax_treatment_metadata,source_type) SELECT $1,$2,$3,$4,$5,$6,version,updated_at,allowance_cents,allowance_cents,payoff_cents,equity_cents,0,'{}','application' FROM trade_appraisals WHERE organization_id=$2 AND id=$5`,[`qts_multi_${i+1}`,org,scenario.quote_id,scenario.id,`tap_multi_${i+1}`,i]);
    const multi=(await q(client,`SELECT sum(equity_cents)::int combined,(SELECT sum(equity_cents)::int FROM trade_appraisals WHERE organization_id=$1 AND id IN ('tap_multi_1','tap_multi_2')) canonical FROM quote_trade_snapshots WHERE organization_id=$1 AND product_scenario_id=$2`,[org,scenario.id])).rows[0];
    await q(client,"COMMIT");
    const beforeRecovery=(await q(client,"SELECT count(*)::int count FROM quote_product_scenarios")).rows[0].count;
    await q(client,"BEGIN"); await q(client,"SELECT set_config('app.organization_id',$1,true)",[org]); await q(client,"INSERT INTO quote_product_scenarios(id,organization_id,quote_id,stable_key,position,product_kind,product_reference,label,source_type) VALUES('qps_recovery_probe',$1,'quo_acceptance_1','recovery:probe',99,'generic','probe','Recovery probe','application')",[org]); await q(client,"ROLLBACK");
    const afterRecovery=(await q(client,"SELECT count(*)::int count FROM quote_product_scenarios")).rows[0].count;
    console.log(JSON.stringify({status:"PASS",sourceIdentity:identity,tempDatabase:name,migrationBefore:"0068",migrationAfter:"0069",fixtureQuotes:pre.length,authorityHashesUnchanged:true,backfill:counts1,replay:counts2,fingerprintStable:true,rls:{tenantVisible,crossRead,authorizedCreation,crossWrite,invalidTenantForeignKey},immutability:{update:immutableUpdate,delete:immutableDelete},incompleteLease,multiTrade:{combined:Number(multi.combined),canonical:Number(multi.canonical),matches:Number(multi.combined)===Number(multi.canonical)},recovery:{strategy:"transactional roll-forward and application rollback compatibility",beforeRecovery,afterRecovery,rolledBack:beforeRecovery===afterRecovery}}));
  } finally { client.release(); }
} finally {
  if(temp) await temp.end();
  try { await admin.query("SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname=$1 AND backend_type='client backend' AND pid<>pg_backend_pid()",[name]); await admin.query(`DROP DATABASE IF EXISTS ${name}`); try { await admin.query("REVOKE quote_vnext_acceptance_runtime FROM CURRENT_USER"); } catch(error) { if(error.code!=="42704") throw error; } await admin.query("DROP ROLE IF EXISTS quote_vnext_acceptance_runtime"); } finally { await admin.end(); }
}
