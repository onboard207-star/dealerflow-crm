...
const args = ["--confirm", "RUN-SYNTHETIC-STAGING-QUOTE-JOURNEY", "--salesperson-email", "sales@example.invalid", "--manager-email", "manager@example.invalid", "--organization-id", "org_demo_first_pilot_v1", "--location-id", "loc_demo_main_rooftop_v1", "--deal-id", "dea_synthetic_deal", "--application-url", "https://staging.example.com", "--expected-database-host", "isolated.internal"];
...
describe("staging Quote journey acceptance", () => {
  ...
  it("requires HTTPS", () => expect(() => parseArguments(args.with(13, "http://staging.example.com"), { APP_ENV: "staging", DATABASE_URL: "postgresql://user:secret@isolated.internal/database" })).toThrow("HTTPS"));
});
