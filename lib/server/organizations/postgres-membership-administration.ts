...
export class PostgresMembershipAdministration implements MembershipAdministrationProvider {
  ...
  updateAccess(input: { actorId:string; organizationId:string; membershipId:string; roleIds:readonly string[]; locationIds:readonly string[]; allLocations:boolean; confirmPrivileged:boolean }) {
    return withTenantDatabaseContext(this.pool,{userId:input.actorId,organizationId:input.organizationId},async(client)=>{
      ...
      const selection=await validateSelection(client as PoolClient,input);if(!selection.valid)return "invalid_scope";if(selection.privileged&&!input.confirmPrivileged)return "privileged_confirmation_required";
      ...
      await audit(client as PoolClient,input,"organization.membership.access_updated",{allLocations:input.allLocations,roleIds:input.roleIds,locationIds:input.allLocations?[]:input.locationIds,privilegedAssignmentConfirmed:selection.privileged}); return "updated";
    });
  }
...
async function validateSelection(client:PoolClient,input:{organizationId:string;roleIds:readonly string[];locationIds:readonly string[];allLocations:boolean}){const result=await client.query<{valid:boolean;privileged:boolean}>("SELECT (SELECT count(*) FROM roles WHERE organization_id=$1 AND id=ANY($2::text[]))=cardinality($2::text[]) AND ($3 OR (SELECT count(*) FROM locations WHERE organization_id=$1 AND active=true AND id=ANY($4::text[]))=cardinality($4::text[])) valid,EXISTS(SELECT 1 FROM roles WHERE organization_id=$1 AND id=ANY($2::text[]) AND key IN ('owner','general-manager','controller','finance-manager')) privileged",[input.organizationId,input.roleIds,input.allLocations,input.locationIds]);return result.rows[0]??{valid:false,privileged:false};}
