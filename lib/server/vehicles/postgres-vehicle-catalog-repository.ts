...
const attributeSelect = `SELECT relationship.configuration_id,relationship.relationship,relationship.conditions,
  attribute.id,attribute.stable_key,attribute.kind,attribute.name,attribute.value,attribute.unit,attribute.readiness,attribute.metadata,
...
interface AttributeRow extends ProvenanceRow { configuration_id:string;kind:CatalogAttribute["kind"];name:string;value:string|null;unit:string|null;readiness:Readiness;metadata:Record<string,unknown>;relationship:CatalogAttribute["relationship"];conditions:Record<string,unknown> }
...
const attribute = (row: AttributeRow): CatalogAttribute => ({id:row.id,stableKey:row.stable_key,name:row.name,kind:row.kind,...(row.value?{value:row.value}:{}),...(row.unit?{unit:row.unit}:{}),metadata:row.metadata,relationship:row.relationship,conditions:row.conditions,...source(row,row.readiness)});
