...
type DealRow = { id: string; organization_id: string; location_id: string; customer_id: string; lead_id: string; appointment_id: string | null; showroom_visit_id: string | null; primary_vehicle_id: string; inventory_unit_id: string | null; owner_user_id: string | null; deal_number: string; status: DealRecord["status"]; purchase_type: DealRecord["purchaseType"] | null; agreed_price_cents: number | null; accepted_quote_id: string | null; accepted_quote_version: number | null; idempotency_key: string };
...
const columns = "id, organization_id, location_id, customer_id, lead_id, appointment_id, showroom_visit_id, primary_vehicle_id, inventory_unit_id, owner_user_id, deal_number, status, purchase_type, agreed_price_cents, accepted_quote_id, accepted_quote_version, idempotency_key";
class Session implements DealSession {
  ...
  async creationContextExists(scope: OrganizationScope, input: { customerId: string; leadId: string; appointmentId?: string; showroomVisitId?: string; vehicleId: string; inventoryUnitId?: string; ownerUserId?: string }) { const result = await this.db.query<{ exists: boolean }>(`SELECT EXISTS (SELECT 1 FROM leads l
      ...
      WHERE l.organization_id = $1 AND l.customer_id = $2 AND l.id = $3
        ...
        AND ($6::text IS NULL OR EXISTS (SELECT 1 FROM organization_memberships m
          JOIN membership_locations ml ON ml.organization_id=m.organization_id AND ml.membership_id=m.id AND ml.location_id=$7
          WHERE m.organization_id = l.organization_id AND m.user_id = $6 AND m.status = 'active'))
        AND ($8::text IS NULL OR EXISTS (SELECT 1 FROM appointments a WHERE a.organization_id=l.organization_id
          AND a.id=$8 AND a.customer_id=l.customer_id AND a.lead_id=l.id AND a.location_id=$7))
        AND ($9::text IS NULL OR EXISTS (SELECT 1 FROM showroom_visits visit WHERE visit.organization_id=l.organization_id
          AND visit.id=$9 AND visit.customer_id=l.customer_id AND visit.lead_id=l.id AND visit.location_id=$7
          AND visit.appointment_id=$8 AND visit.status IN ('checked-in','active','completed')))) AS exists`, [scope.organizationId, input.customerId, input.leadId, input.vehicleId, input.inventoryUnitId ?? null, input.ownerUserId ?? null, scope.locationId ?? null, input.appointmentId ?? null, input.showroomVisitId ?? null]); return result.rows[0]?.exists === true; }
  async createDeal(context: RequestContext, record: DealRecord, event: DealStatusEvent) { try { await this.db.query(`INSERT INTO deals (id, organization_id, location_id, customer_id, lead_id, appointment_id, showroom_visit_id, primary_vehicle_id,
  ...
  async acceptedQuoteForContract(scope: OrganizationScope, dealId: string) { const result = await this.db.query<{ id: string; version: number; purchase_type: NonNullable<DealRecord["purchaseType"]>; total_cents: number }>(`SELECT id, version, purchase_type, total_cents FROM deal_quotes WHERE organization_id=$1 AND deal_id=$2 AND status='accepted' FOR UPDATE`, [scope.organizationId, dealId]); const row=result.rows[0]; return row ? { quoteId: row.id, version: row.version, purchaseType: row.purchase_type, totalCents: row.total_cents } : null; }
  async deliveryReady(scope: OrganizationScope, dealId: string) { const result=await this.db.query<{ ready:boolean }>(`SELECT EXISTS (SELECT 1 FROM deals d JOIN deal_quotes q ON q.organization_id=d.organization_id AND q.id=d.accepted_quote_id AND q.deal_id=d.id AND q.version=d.accepted_quote_version AND q.status='accepted' WHERE d.organization_id=$1 AND d.id=$2 AND d.status='contracted' AND (d.inventory_unit_id IS NULL OR EXISTS (SELECT 1 FROM inventory_units i WHERE i.organization_id=d.organization_id AND i.location_id=d.location_id AND i.id=d.inventory_unit_id AND i.vehicle_id=d.primary_vehicle_id AND i.status='hold')) AND EXISTS (SELECT 1 FROM deal_document_requirements r WHERE r.organization_id=d.organization_id AND r.deal_id=d.id AND r.required) AND NOT EXISTS (SELECT 1 FROM deal_document_requirements r WHERE r.organization_id=d.organization_id AND r.deal_id=d.id AND r.required AND r.status NOT IN ('complete','waived'))) AS ready`,[scope.organizationId,dealId]); return result.rows[0]?.ready===true; }
  async transitionDeal(context: RequestContext, record: DealRecord, statusEvent: DealStatusEvent) {
    ...
    const update = await this.db.query<{ id: string }>(`UPDATE deals SET status = $3, purchase_type=$6, agreed_price_cents=$7, accepted_quote_id=$8, accepted_quote_version=$9, updated_by = $4, updated_at = now() WHERE organization_id = $1 AND id = $2 AND status = $5 RETURNING id`, [record.organizationId, record.id, record.status, context.actorId, statusEvent.fromStatus, record.purchaseType ?? null, record.agreedPriceCents ?? null, record.acceptedQuoteId ?? null, record.acceptedQuoteVersion ?? null]); if (!update.rows[0]) throw new Error("Concurrent deal transition was rejected.");
    if (record.status === "contracted" && record.acceptedQuoteId && record.acceptedQuoteVersion) await this.db.query(`INSERT INTO deal_document_requirements (id,organization_id,location_id,deal_id,quote_id,quote_version,document_type,document_version,provenance,source_type,status,required,idempotency_key,completed_by,completed_at,created_by) VALUES($1,$2,$3,$4,$5,$6,'accepted-quote',$6,'canonical-quote','canonical-quote','complete',true,$7,$8,now(),$8)`, [generateEntityId("ddr"), record.organizationId, record.locationId, record.id, record.acceptedQuoteId, record.acceptedQuoteVersion, `contract:${statusEvent.id}:accepted-quote`, context.actorId]);
    if (record.status === "delivered") {
