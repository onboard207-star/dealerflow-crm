...
export const vehicleCatalogAttributes=pgTable("vehicle_catalog_attributes",{id:text("id").primaryKey(),stableKey:text("stable_key").notNull(),kind:text("kind").notNull(),name:text("name").notNull(),value:text("value"),unit:text("unit"),metadata:jsonb("metadata").$type<Record<string,unknown>>().default({}).notNull(),...catalogProvenance},(table)=>[uniqueIndex("vehicle_catalog_attributes_stable_key_unique").on(table.stableKey),index("vehicle_catalog_attributes_kind_name_idx").on(table.kind,table.name),check("vehicle_catalog_attributes_id_format",sql`${table.id} ~ '^vca_[a-f0-9]{32}$'`)]);
