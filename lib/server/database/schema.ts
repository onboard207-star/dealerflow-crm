...
export const vehicleCatalogReleases=pgTable("vehicle_catalog_releases",{id:text("id").primaryKey(),sourceSystem:text("source_system").notNull(),sourceDataset:text("source_dataset").notNull(),sourceRevision:text("source_revision").notNull(),manifestSha256:text("manifest_sha256").notNull(),manifest:jsonb("manifest").$type<Record<string,unknown>>().notNull(),status:text("status").default("staged").notNull(),recordCounts:jsonb("record_counts").$type<Record<string,number>>().default({}).notNull(),startedAt:timestamp("started_at",{withTimezone:true}).defaultNow().notNull(),completedAt:timestamp("completed_at",{withTimezone:true}),createdAt:timestamp("created_at",{withTimezone:true}).defaultNow().notNull()},(table)=>[uniqueIndex("vehicle_catalog_releases_source_revision_unique").on(table.sourceSystem,table.sourceDataset,table.sourceRevision),uniqueIndex("vehicle_catalog_releases_manifest_unique").on(table.manifestSha256),check("vehicle_catalog_releases_id_format",sql`${table.id} ~ '^vcr_[a-z0-9_-]{6,64}$'`),check("vehicle_catalog_releases_sha256",sql`${table.manifestSha256} ~ '^[a-f0-9]{64}$'`),check("vehicle_catalog_releases_status",sql`${table.status} in ('staged','accepted','rejected','superseded')`)]);

const catalogProvenance={releaseId:text("release_id").notNull().references(()=>vehicleCatalogReleases.id,{onDelete:"restrict"}),sourceSystem:text("source_system").notNull(),sourceRecordId:text("source_record_id"),contentSha256:text("content_sha256").notNull(),...timestamps};

export const vehicleCatalogMakes=pgTable("vehicle_catalog_makes",{id:text("id").primaryKey(),stableKey:text("stable_key").notNull(),name:text("name").notNull(),status:text("status").default("active").notNull(),...catalogProvenance},(table)=>[uniqueIndex("vehicle_catalog_makes_stable_key_unique").on(table.stableKey),check("vehicle_catalog_makes_id_format",sql`${table.id} ~ '^vma_[a-f0-9]{32}$'`)]);
export const vehicleCatalogModels=pgTable("vehicle_catalog_models",{id:text("id").primaryKey(),makeId:text("make_id").notNull().references(()=>vehicleCatalogMakes.id,{onDelete:"restrict"}),stableKey:text("stable_key").notNull(),name:text("name").notNull(),vehicleClass:text("vehicle_class"),status:text("status").default("active").notNull(),...catalogProvenance},(table)=>[uniqueIndex("vehicle_catalog_models_stable_key_unique").on(table.stableKey),index("vehicle_catalog_models_make_idx").on(table.makeId,table.name),check("vehicle_catalog_models_id_format",sql`${table.id} ~ '^vmo_[a-f0-9]{32}$'`)]);
export const vehicleCatalogModelYears=pgTable("vehicle_catalog_model_years",{id:text("id").primaryKey(),modelId:text("model_id").notNull().references(()=>vehicleCatalogModels.id,{onDelete:"restrict"}),stableKey:text("stable_key").notNull(),year:integer("year").notNull(),generation:text("generation"),readiness:text("readiness").default("needs-review").notNull(),...catalogProvenance},(table)=>[uniqueIndex("vehicle_catalog_model_years_stable_key_unique").on(table.stableKey),uniqueIndex("vehicle_catalog_model_years_model_year_unique").on(table.modelId,table.year),check("vehicle_catalog_model_years_id_format",sql`${table.id} ~ '^vmy_[a-f0-9]{32}$'`)]);
export const vehicleCatalogTrims=pgTable("vehicle_catalog_trims",{id:text("id").primaryKey(),modelYearId:text("model_year_id").notNull().references(()=>vehicleCatalogModelYears.id,{onDelete:"restrict"}),stableKey:text("stable_key").notNull(),name:text("name").notNull(),badge:text("badge"),readiness:text("readiness").default("needs-review").notNull(),...catalogProvenance},(table)=>[uniqueIndex("vehicle_catalog_trims_stable_key_unique").on(table.stableKey),index("vehicle_catalog_trims_model_year_idx").on(table.modelYearId,table.name),check("vehicle_catalog_trims_id_format",sql`${table.id} ~ '^vtr_[a-f0-9]{32}$'`)]);
export const vehicleCatalogConfigurations=pgTable("vehicle_catalog_configurations",{id:text("id").primaryKey(),trimId:text("trim_id").notNull().references(()=>vehicleCatalogTrims.id,{onDelete:"restrict"}),stableKey:text("stable_key").notNull(),name:text("name").notNull(),drivetrain:text("drivetrain"),powertrain:text("powertrain"),engine:text("engine"),transmission:text("transmission"),bodyStyle:text("body_style"),seatCount:integer("seat_count"),readiness:text("readiness").default("needs-review").notNull(),...catalogProvenance},(table)=>[uniqueIndex("vehicle_catalog_configurations_stable_key_unique").on(table.stableKey),index("vehicle_catalog_configurations_trim_idx").on(table.trimId,table.readiness),check("vehicle_catalog_configurations_id_format",sql`${table.id} ~ '^vcf_[a-f0-9]{32}$'`)]);
export const vehicleCatalogAttributes=pgTable("vehicle_catalog_attributes",{id:text("id").primaryKey(),stableKey:text("stable_key").notNull(),kind:text("kind").notNull(),name:text("name").notNull(),value:text("value"),unit:text("unit"),readiness:text("readiness").default("needs-review").notNull(),metadata:jsonb("metadata").$type<Record<string,unknown>>().default({}).notNull(),...catalogProvenance},(table)=>[uniqueIndex("vehicle_catalog_attributes_stable_key_unique").on(table.stableKey),index("vehicle_catalog_attributes_kind_name_idx").on(table.kind,table.name),check("vehicle_catalog_attributes_id_format",sql`${table.id} ~ '^vca_[a-f0-9]{32}$'`)]);
export const vehicleCatalogConfigurationAttributes=pgTable("vehicle_catalog_configuration_attributes",{configurationId:text("configuration_id").notNull().references(()=>vehicleCatalogConfigurations.id,{onDelete:"cascade"}),attributeId:text("attribute_id").notNull().references(()=>vehicleCatalogAttributes.id,{onDelete:"restrict"}),relationship:text("relationship").default("standard").notNull(),conditions:jsonb("conditions").$type<Record<string,unknown>>().default({}).notNull(),releaseId:text("release_id").notNull().references(()=>vehicleCatalogReleases.id,{onDelete:"restrict"}),sourceRecordId:text("source_record_id"),createdAt:timestamp("created_at",{withTimezone:true}).defaultNow().notNull()},(table)=>[primaryKey({columns:[table.configurationId,table.attributeId],name:"vehicle_catalog_configuration_attributes_pk"}),index("vehicle_catalog_configuration_attributes_attribute_idx").on(table.attributeId,table.configurationId)]);
export const vehicleCatalogMatches=pgTable("vehicle_catalog_matches",{id:text("id").primaryKey(),organizationId:text("organization_id").notNull(),vehicleId:text("vehicle_id").notNull(),configurationId:text("configuration_id").notNull().references(()=>vehicleCatalogConfigurations.id,{onDelete:"restrict"}),status:text("status").default("proposed").notNull(),evidence:jsonb("evidence").$type<readonly Record<string,unknown>[]>().default([]).notNull(),source:text("source").notNull(),matchedBy:text("matched_by").references(()=>users.id,{onDelete:"set null"}),matchedAt:timestamp("matched_at",{withTimezone:true}).defaultNow().notNull(),supersededAt:timestamp("superseded_at",{withTimezone:true}),createdAt:timestamp("created_at",{withTimezone:true}).defaultNow().notNull()},(table)=>[uniqueIndex("vehicle_catalog_matches_org_id_unique").on(table.organizationId,table.id),uniqueIndex("vehicle_catalog_matches_one_verified").on(table.organizationId,table.vehicleId).where(sql`${table.status}='verified'`),index("vehicle_catalog_matches_configuration_idx").on(table.configurationId,table.status),foreignKey({columns:[table.organizationId,table.vehicleId],foreignColumns:[vehicles.organizationId,vehicles.id],name:"vehicle_catalog_matches_same_vehicle_fk"}),check("vehicle_catalog_matches_id_format",sql`${table.id} ~ '^vcm_[a-z0-9_-]{6,64}$'`)]);
...
},table=>[uniqueIndex("notifications_organization_id_unique").on(table.organizationId,table.id),uniqueIndex("notifications_dedupe_unique").on(table.organizationId,table.dedupeKey),index("notifications_recipient_unread_idx").on(table.organizationId,table.recipientUserId,table.createdAt).where(sql`${table.readAt} is null`),foreignKey({columns:[table.organizationId,table.locationId],foreignColumns:[locations.organizationId,locations.id],name:"notifications_same_location_fk"}),check("notifications_id_format",sql`${table.id} ~ '^ntf_[a-z0-9_-]{6,64}$'`),check("notifications_kind",sql`${table.kind} in ('task-assigned','deal-approval','team-message')`),check("notifications_content_length",sql`length(trim(${table.title})) between 1 and 200 and length(trim(${table.body})) between 1 and 500`),check("notifications_href_internal",sql`${table.href} like '/organizations/%'`)]);

export const teamConversations=pgTable("team_conversations",{
  id:text("id").primaryKey(),organizationId:text("organization_id").notNull().references(()=>organizations.id,{onDelete:"cascade"}),locationId:text("location_id"),kind:text("kind").notNull(),name:text("name"),createdBy:text("created_by").notNull().references(()=>users.id,{onDelete:"restrict"}),participantUserIds:text("participant_user_ids").array().notNull(),idempotencyKey:text("idempotency_key").notNull(),archivedAt:timestamp("archived_at",{withTimezone:true}),...timestamps,
},table=>[uniqueIndex("team_conversations_org_id_unique").on(table.organizationId,table.id),uniqueIndex("team_conversations_idempotency_unique").on(table.organizationId,table.idempotencyKey),index("team_conversations_location_updated_idx").on(table.organizationId,table.locationId,table.updatedAt),foreignKey({columns:[table.organizationId,table.locationId],foreignColumns:[locations.organizationId,locations.id],name:"team_conversations_same_location_fk"}),check("team_conversations_id_format",sql`${table.id} ~ '^cnv_[a-z0-9_-]{6,64}$'`),check("team_conversations_kind",sql`${table.kind} in ('direct','group','department','record')`)]);
export const teamConversationParticipants=pgTable("team_conversation_participants",{
  organizationId:text("organization_id").notNull(),conversationId:text("conversation_id").notNull(),userId:text("user_id").notNull().references(()=>users.id,{onDelete:"cascade"}),role:text("role").default("member").notNull(),joinedAt:timestamp("joined_at",{withTimezone:true}).defaultNow().notNull(),lastReadAt:timestamp("last_read_at",{withTimezone:true}),
},table=>[primaryKey({columns:[table.conversationId,table.userId]}),index("team_participants_user_idx").on(table.organizationId,table.userId,table.conversationId),foreignKey({columns:[table.organizationId,table.conversationId],foreignColumns:[teamConversations.organizationId,teamConversations.id],name:"team_participants_same_conversation_fk"})]);
export const teamConversationParticipantEvents=pgTable("team_conversation_participant_events",{
  id:text("id").primaryKey(),organizationId:text("organization_id").notNull(),conversationId:text("conversation_id").notNull(),actorUserId:text("actor_user_id").notNull().references(()=>users.id,{onDelete:"restrict"}),addedUserId:text("added_user_id").notNull().references(()=>users.id,{onDelete:"restrict"}),oldParticipantUserIds:text("old_participant_user_ids").array().notNull(),newParticipantUserIds:text("new_participant_user_ids").array().notNull(),idempotencyKey:text("idempotency_key").notNull(),createdAt:timestamp("created_at",{withTimezone:true}).defaultNow().notNull(),
},table=>[uniqueIndex("team_participant_events_org_id_unique").on(table.organizationId,table.id),uniqueIndex("team_participant_events_idempotency_unique").on(table.organizationId,table.conversationId,table.idempotencyKey),index("team_participant_events_conversation_time_idx").on(table.organizationId,table.conversationId,table.createdAt),foreignKey({columns:[table.organizationId,table.conversationId],foreignColumns:[teamConversations.organizationId,teamConversations.id],name:"team_participant_events_same_conversation_fk"}),check("team_participant_events_id_format",sql`${table.id} ~ '^tpe_[a-z0-9_-]{6,64}$'`)]);
export const teamMessages=pgTable("team_messages",{
  id:text("id").primaryKey(),organizationId:text("organization_id").notNull(),conversationId:text("conversation_id").notNull(),senderUserId:text("sender_user_id").notNull().references(()=>users.id,{onDelete:"restrict"}),body:text("body").notNull(),kind:text("kind").default("text").notNull(),idempotencyKey:text("idempotency_key").notNull(),createdAt:timestamp("created_at",{withTimezone:true}).defaultNow().notNull(),
},table=>[uniqueIndex("team_messages_org_id_unique").on(table.organizationId,table.id),uniqueIndex("team_messages_idempotency_unique").on(table.organizationId,table.conversationId,table.idempotencyKey),index("team_messages_conversation_time_idx").on(table.organizationId,table.conversationId,table.createdAt,table.id),foreignKey({columns:[table.organizationId,table.conversationId],foreignColumns:[teamConversations.organizationId,teamConversations.id],name:"team_messages_same_conversation_fk"})]);
export const teamMessageReferences=pgTable("team_message_references",{
  id:text("id").primaryKey(),organizationId:text("organization_id").notNull(),messageId:text("message_id").notNull(),entityType:text("entity_type").notNull(),entityId:text("entity_id").notNull(),label:text("label").notNull(),href:text("href").notNull(),createdAt:timestamp("created_at",{withTimezone:true}).defaultNow().notNull(),
},table=>[uniqueIndex("team_message_references_message_unique").on(table.organizationId,table.messageId,table.entityType,table.entityId),foreignKey({columns:[table.organizationId,table.messageId],foreignColumns:[teamMessages.organizationId,teamMessages.id],name:"team_message_references_same_message_fk"}),check("team_message_references_id_format",sql`${table.id} ~ '^mrf_[a-z0-9_-]{6,64}$'`)]);
