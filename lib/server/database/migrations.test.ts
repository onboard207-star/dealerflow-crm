...
it("restores forced RLS after migration-time legacy backfill visibility",()=>{const migration=readFileSync(join(migrationDirectory,"0069_quote_vnext_scenarios.sql"),"utf8");expect(migration.match(/NO FORCE ROW LEVEL SECURITY/g)?.length).toBe(12);expect(migration.match(/FORCE ROW LEVEL SECURITY/g)?.length).toBeGreaterThanOrEqual(15);expect(migration).toContain("temporarily remove FORCE only from the");});
