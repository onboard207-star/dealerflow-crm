...
describe("tenant database migrations", () => {
  ...
  it("adds a governed global vehicle catalog without collapsing physical inventory",()=>{const migration=readFileSync(join(migrationDirectory,"0054_vehicle_intelligence_catalog.sql"),"utf8");expect(migration).toContain('CREATE TABLE "vehicle_catalog_configurations"');expect(migration).toContain('CREATE TABLE "vehicle_catalog_attributes"');expect(migration).toContain('CREATE TABLE "vehicle_catalog_matches"');expect(migration).toContain("app.vehicle_catalog_import");expect(migration).toContain("vehicle_catalog_matches_same_vehicle_fk");expect(migration).toContain("vehicle_catalog_matches_one_verified");expect(migration).not.toMatch(/ALTER TABLE "vehicles" ADD COLUMN/);});
  ...
  it("preserves governed readiness for projected catalog attributes",()=>{const migration=readFileSync(join(migrationDirectory,"0068_vehicle_catalog_attribute_readiness.sql"),"utf8");expect(migration).toContain('ADD COLUMN "readiness"');expect(migration).toContain("jsonb_array_elements");expect(migration).toContain("pilot-ready");expect(migration).not.toMatch(/vin|stock_number|inventory_unit/);});
  it("adds immutable tenant-scoped Quote vNext scenarios without rewriting legacy authority",()=>{const migration=readFileSync(join(migrationDirectory,"0069_quote_vnext_scenarios.sql"),"utf8");for(const table of["quote_product_scenarios","quote_payment_scenarios","quote_trade_snapshots"]){expect(migration).toContain(`CREATE TABLE "${table}"`);expect(migration).toContain(`ALTER TABLE "${table}" FORCE ROW LEVEL SECURITY`);expect(migration).toContain(`CREATE POLICY "${table}`);}expect(migration).toContain("quote_payment_scenarios_fingerprint");expect(migration).toContain("quote_trade_snapshots_adjustment");expect(migration).toContain("prevent_quote_vnext_child_rewrite");expect(migration).not.toMatch(/UPDATE\s+"?deal_quotes"?/i);expect(migration).not.toMatch(/UPDATE\s+"?trade_appraisals"?/i);});
it("backfills one deterministic default scenario and reconciles legacy Quote evidence",()=>{const migration=readFileSync(join(migrationDirectory,"0069_quote_vnext_scenarios.sql"),"utf8");expect(migration).toContain("quote_vnext_legacy_baseline");expect(migration).toContain("legacy:default-product");expect(migration).toContain("legacy-backfill-v1");expect(migration).toContain("ON CONFLICT (organization_id,quote_id,stable_key) DO NOTHING");expect(migration).toContain("ON CONFLICT (organization_id,product_scenario_id,trade_appraisal_id) DO NOTHING");expect(migration).toContain("ON CONFLICT (organization_id,product_scenario_id,stable_key) DO NOTHING");expect(migration).toContain("legacy financial or lifecycle evidence changed");expect(migration).toContain("legacy Quote scenario cardinality is invalid");expect(migration).toContain("legacy Trade snapshot cardinality is invalid");expect(migration).toContain("incomplete provider terms");});
it("restores forced RLS after migration-time legacy backfill visibility",()=>{const migration=readFileSync(join(migrationDirectory,"0069_quote_vnext_scenarios.sql"),"utf8");expect(migration.match(/NO FORCE ROW LEVEL SECURITY/g)?.length).toBe(12);expect(migration.match(/DISABLE ROW LEVEL SECURITY/g)?.length).toBe(12);expect(migration.match(/ENABLE ROW LEVEL SECURITY/g)?.length).toBeGreaterThanOrEqual(15);expect(migration.match(/FORCE ROW LEVEL SECURITY/g)?.length).toBeGreaterThanOrEqual(15);expect(migration).toContain("temporarily remove FORCE only from the");});
  ...
  it("adds governed atomic team-message notification fan-out",()=>{const migration=readFileSync(join(migrationDirectory,"0065_team_message_notification_fanout.sql"),"utf8");expect(migration).toContain("CREATE FUNCTION create_team_message_notifications");expect(migration).toContain("SECURITY DEFINER");expect(migration).toContain("participant_user_ids");expect(migration).toContain("membership.status='active'");expect(migration).toContain("ON CONFLICT(organization_id,dedupe_key) DO NOTHING");expect(migration).not.toContain("message.body");});
  it("keeps team-message notification insertion inside ordinary row security",()=>{const migration=readFileSync(join(migrationDirectory,"0066_team_message_notification_rls.sql"),"utf8");expect(migration).toContain("DROP FUNCTION create_team_message_notifications");expect(migration).toContain("notifications.recipient_user_id=ANY(conversation.participant_user_ids)");expect(migration).toContain("message.sender_user_id=nullif(current_setting('app.user_id',true),'')");expect(migration).not.toMatch(/DISABLE ROW LEVEL SECURITY|NO FORCE ROW LEVEL SECURITY|SECURITY DEFINER/);});
