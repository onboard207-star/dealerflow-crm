...
export class SignedAlertWebhook {
  ...
  async deliver(event:TelemetryEvent):Promise<void>{
    ...
    const payload=JSON.stringify({version:1,timestamp:this.now().toISOString(),text:`DealerFlow ${safe.severity.toUpperCase()}: ${safe.code} · ${safe.correlationId}`,event:{code:safe.code,severity:safe.severity,correlationId:safe.correlationId,...(safe.organizationId?{organizationId:safe.organizationId}:{}),attributes:safe.attributes??{}}});
