...
describe("public marketing boundary",()=>{
  ...
  it("does not claim SMS availability before the external provider gate passes",()=>{const source=read("app/integrations/page.tsx");expect(source).toContain("provider approval pending");expect(source).toContain("Staging SMS remains unavailable");expect(source).not.toContain("Twilio communications — available");});
});
