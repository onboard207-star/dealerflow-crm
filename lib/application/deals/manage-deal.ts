...
export interface DealRecord extends OrganizationScope { id: string; customerId: string; leadId: string; primaryVehicleId: string; inventoryUnitId?: string; ownerUserId?: string; dealNumber: string; status: DealStatus; purchaseType?: PurchaseType; agreedPriceCents?: number; idempotencyKey: string; }
...
export interface CreateDealRequest extends OrganizationScope { actor: AuthorizationActor; correlationId: string; idempotencyKey: string; customerId: string; leadId: string; primaryVehicleId: string; inventoryUnitId?: string; ownerUserId?: string; purchaseType?: PurchaseType; agreedPriceCents?: number; }
...
export interface DealSession {
  ...
  creationContextExists(scope: OrganizationScope, input: { customerId: string; leadId: string; vehicleId: string; inventoryUnitId?: string; ownerUserId?: string }): Promise<boolean>;
...
export class DealService {
  ...
  async create(request: CreateDealRequest): Promise<{ deal: DealRecord; created: boolean }> {
    ...
    return this.provider.transaction(async (session) => { await session.acquireIdempotencyLock(request, request.idempotencyKey); const existing = await session.findDealByIdempotency(request, request.idempotencyKey); if (existing) return { deal: existing, created: false };
      if (!await session.creationContextExists(request, { customerId: request.customerId, leadId: request.leadId, vehicleId: request.primaryVehicleId, ...(request.inventoryUnitId ? { inventoryUnitId: request.inventoryUnitId } : {}), ...(request.ownerUserId ? { ownerUserId: request.ownerUserId } : {}) })) throw new DealIntegrityError("The customer, lead, vehicle, inventory, or owner context is unavailable.");
      const id = generateEntityId("dea"); const deal: DealRecord = { id, organizationId: request.organizationId, locationId: request.locationId!, customerId: request.customerId, leadId: request.leadId, primaryVehicleId: request.primaryVehicleId,
...
function validateCreate(request: CreateDealRequest) { const issues: string[] = []; if (!request.locationId) issues.push("locationId is required."); for (const [field, value] of [["customerId", request.customerId], ["leadId", request.leadId], ["primaryVehicleId", request.primaryVehicleId], ["idempotencyKey", request.idempotencyKey]] as const) if (!value.trim()) issues.push(`${field} is required.`); if (request.agreedPriceCents !== undefined && (!Number.isSafeInteger(request.agreedPriceCents) || request.agreedPriceCents < 0)) issues.push("agreedPriceCents must be a nonnegative integer."); if (issues.length) throw new DealValidationError(issues); }
