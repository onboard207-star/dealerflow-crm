...
import { QuoteIntegrityError, QuoteService, QuoteTransitionError, QuoteValidationError,
  type CreateQuoteRequest, type QuoteProvider, type QuoteRecord, type QuoteSession, type QuoteStatusEvent, type TransitionQuoteRequest } from "./manage-quote";
...
class MemoryProvider implements QuoteProvider, QuoteSession {
  ...
  async getDealContextForUpdate() { return { status: this.dealStatus, locationId: this.locationId }; }
  async nextVersion(_scope: { organizationId: string }, dealId: string) { return this.quotes.filter((item) => item.dealId === dealId).length + 1; }
...
describe("QuoteService", () => {
  it("calculates an immutable quote version from typed line items", async () => { const provider = new MemoryProvider(); const service = new QuoteService(provider, () => new Date("2026-08-23T12:00:00.000Z")); const first = await service.create(createRequest()); const retry = await service.create(createRequest()); expect(first.created).toBe(true); expect(retry.created).toBe(false); expect(first.quote).toMatchObject({ version: 1, subtotalCents: 4450000, feeCents: 49900, taxCents: 260000, discountCents: -100000, totalCents: 4659900 }); expect(provider.quotes).toHaveLength(1); });
  it("allocates a new version without rewriting the first quote", async () => { const provider = new MemoryProvider(); const service = new QuoteService(provider, () => new Date("2026-08-23T12:00:00.000Z")); const first = await service.create(createRequest()); const second = await service.create(createRequest({ idempotencyKey: "quote:deal-1:v2", lines: [{ category: "vehicle", description: "CR-V", unitAmountCents: 4100000 }] })); expect(first.quote.version).toBe(1); expect(second.quote.version).toBe(2); expect(first.quote.totalCents).toBe(4659900); });
  it("requires exactly one vehicle line and safe signed arithmetic", async () => { const service = new QuoteService(new MemoryProvider()); await expect(service.create(createRequest({ lines: [{ category: "discount", description: "Wrong sign", unitAmountCents: 100 }] }))).rejects.toBeInstanceOf(QuoteValidationError); });
  it("presents and accepts a current quote through controlled transitions", async () => { const provider = new MemoryProvider(); const service = new QuoteService(provider, () => new Date("2026-08-23T12:00:00.000Z")); const { quote } = await service.create(createRequest()); await service.transition(transitionRequest(quote.id, "presented")); const result = await service.transition(transitionRequest(quote.id, "accepted")); expect(result.quote.status).toBe("accepted"); expect(result.quote.acceptedAt).toBe("2026-08-23T12:00:00.000Z"); });
