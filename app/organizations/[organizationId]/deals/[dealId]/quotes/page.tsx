              ...
              <ul className="mt-4 space-y-4" role="list">
                {workspace.quotes.map((quote) => (
                  <li className="rounded-xl border bg-card p-4 shadow-soft sm:p-5" key={quote.id}>
                    ...
                    {quote.status === "draft" && canViewSensitive && !quote.profitability ? <details className="mt-4 border-t pt-4"><summary className="focus-ring cursor-pointer rounded-sm text-sm font-semibold">Capture sourced vehicle cost and profitability</summary><form action={captureQuoteProfitabilityAction.bind(null, organizationId, dealId, quote.id)} className="mt-4 grid gap-4 sm:grid-cols-2"><MoneyField label="Authoritative vehicle cost" name="vehicleCost" required /><Field label="Cost source type"><select className="focus-ring h-10 w-full rounded-lg border bg-background px-3 text-sm" name="costSourceType" defaultValue="" required><option value="">Choose source</option><option value="dms">DMS</option><option value="accounting">Accounting</option><option value="invoice">Invoice</option><option value="acquisition">Acquisition record</option><option value="manual-documented">Manual documented source</option></select></Field><Field label="Source label"><input className="focus-ring h-10 w-full rounded-lg border bg-background px-3 text-sm" maxLength={200} name="costSourceLabel" required /></Field><Field label="Source reference"><input className="focus-ring h-10 w-full rounded-lg border bg-background px-3 text-sm" maxLength={500} name="costSourceReference" /></Field><Field label="Cost effective at"><input className="focus-ring h-10 w-full rounded-lg border bg-background px-3 text-sm" name="costEffectiveAt" type="datetime-local" required /></Field><p className="rounded-lg border bg-muted/20 p-3 text-xs text-muted-foreground sm:col-span-2">DealerFlow uses only the documented cost entered here and the configured location or organization pack policy. No MSRP, selling price, book value, or appraisal value is substituted for cost.</p><button className="focus-ring min-h-10 rounded-lg bg-primary px-4 text-sm font-semibold text-primary-foreground sm:col-span-2">Capture immutable profitability</button></form></details> : null}
                  </li>
                ))}
              </ul>
            ) : (
