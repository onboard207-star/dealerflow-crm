...
export async function POST(request: Request, context: Context) {
  try {
    ...
    const result = await new OutboundMessagingService(
    ...
    ).send({ actor, organizationId, ...(locationId ? { locationId } : {}), customerId,
      ...(optionalString(body.leadId) ? { leadId: optionalString(body.leadId) } : {}),
      integrationId: stringValue(body.integrationId), destination: stringValue(body.destination),
      body: stringValue(body.body),
      purpose: enumValue(body.purpose, ["operational", "marketing"] as const) ?? "operational",
      idempotencyKey, requestedAt: new Date().toISOString(),
      correlationId: request.headers.get("x-correlation-id")?.trim() || `req_${randomUUID()}` });
    const status = result.attempt.status === "queued" || result.attempt.status === "delivery-unknown" ? 202 : 200;
...
function failure(error: unknown) {
  ...
  if (error instanceof MembershipError || error instanceof AuthorizationError) return problem(403, "forbidden", "Outbound messaging is not permitted.");
  ...
  return problem(500, "internal_error", "The message could not be prepared.");
}
