...
- The protected transactional-email worker supplied one safe controlled warning path from an existing staging-blocked message: HTTP 200 with `claimed:1`, `sent:0`, and `failed:1`. Render emitted sanitized `email.job.completed` warning telemetry with a correlation ID and no `operations.alert.delivery_failed` event, proving the external receiver accepted delivery without contacting Resend. Human-visible acknowledgement in the named Slack channel and a named escalation owner remain required before `EVD-ALERT-001` can become current.
