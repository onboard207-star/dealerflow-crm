...
- No catalog release was published: staging contains zero release rows and zero accepted releases. No physical Vehicle or Inventory data was changed.
- The isolated database currently contains zero Organizations, which is inconsistent with the previously accepted synthetic journey and blocks controlled Vehicle matching plus role UI acceptance until the staging dataset is reconciled through the governed seed/recovery path.
- Local, uncommitted acceptance repairs add a staging-only job-authenticated projection route, complete color eligibility/rule extraction, provider-ID stripping from canonical content, and correct configuration field mapping. These changes pass lint, strict typecheck, 656 tests, production build, and whitespace validation, but remain uncommitted while source reconciliation is blocked.
