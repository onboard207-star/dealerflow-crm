...
Verified browser uploads are classified as `actual` inventory photos. Media metadata separately records source type, media type, original filename, active primary state, display order, verification actor, checksum, dimensions, and exact inventory ownership. CGI and OEM reference sources are reserved for explicitly governed ingestion paths and must remain visibly labeled; they are never inferred from year/make/model alone.
