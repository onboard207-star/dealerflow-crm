...
Migration `0068_vehicle_catalog_attribute_readiness` closes a runtime provenance gap exposed by the expanded EV taxonomy. Attribute readiness is now persisted from the governed release manifest rather than being treated as verified by default. Existing accepted attributes are backfilled from their immutable release manifests, and subsequent projections update readiness with the attribute content. Comparison facts are verified only when every contributing attribute is `verified` or `pilot-ready`; otherwise they are explicitly incomplete. EPA electric range, charging, connector, MPG, and MPGe are matched as distinct facts, and an absent fact remains unavailable rather than becoming zero or borrowing a neighboring metric.
