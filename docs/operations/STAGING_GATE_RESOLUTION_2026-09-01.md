...
## Fixture-context correction and third guarded reset

Code-only fix `715f3f9489e31a33ea4387b2108f54a5b69c4b06` passed 498 tests, strict TypeScript, lint, Drizzle migration validation, portfolio and execution validation, production build, and whitespace validation. No migration `0041` was created. The fix explicitly propagates governed manifest version `pilot-demo-v1` into the reset transaction and fails before DELETE for missing, malformed, or noncanonical authorization context while preserving internal template version `v1`, deterministic IDs, and stored idempotency keys.

The exact fix commit deployed successfully to isolated service `srv-dabk8d610ojc73ds3afg`. Preflight reconfirmed `APP_ENV=staging`, database host `dpg-dabjm5e1egvs73b1s92g-a`, absent production provider credentials, and three-day point-in-time recovery. A read-only transaction proved `app.organization_id=org_demo_first_pilot_v1` and `app.synthetic_fixture_version=pilot-demo-v1`, not `v1`, with the canonical 26/1,476/432/48 fixture baseline and zero prior reset-completed events.

The single authorized reset then failed closed with `permission denied for function notification_recipient_is_active` under the dedicated maintenance login. No reseed ran. Rollback verification returned 26 staff, 1,476 Leads, 432 delivered Deals, 48 current Inventory Units, 432 deliveries, 432 Deal status events, and zero `synthetic.reset_completed` events. Both protected lifecycle tables still have enabled and forced RLS. The temporary maintenance login was returned to `NOLOGIN`, and its credential file was removed.

This is the current P1 blocker. The exact next gate is read-only dependency analysis for `notification_recipient_is_active`, followed by a reviewed least-privilege migration and explicit authorization for another single reset attempt. Production, legacy staging, providers, non-demo tenants, and real communications remain untouched and unauthorized.
