...
| --- | --- | --- | --- | --- |
| PILOT-P1-01 | P1 — BLOCKED | Communications | Deterministic staging safety, STOP, callback-idempotency, and failure behavior are repaired and tested, but isolated staging has no Twilio/Resend credentials, sender identities, callback URL, or controlled recipient allowlists. No provider transaction occurred. See [restricted staging acceptance](STAGING_COMMUNICATIONS_ACCEPTANCE_2026-09-07.md). | Approve the pilot channels, configure sandbox/restricted staging routes and exact test destinations, then pass real outbound, inbound, status, failure, retry, consent, duplicate, outage, timeline, authorization, and responsive acceptance without contacting real customers. |
| PILOT-P1-02 | P1 | Vehicle intelligence and inventory scope | Physical VIN/stock Inventory works, but the governed catalog-to-runtime projection for configurations, trims, colors, packages, features, and specifications is not accepted. | If those capabilities are promised in pilot scope, complete source QA and a versioned DealerFlow-ID projection with provenance, reconciliation, failure handling, and no Airtable record IDs in runtime authority. Otherwise record the reduced Inventory scope explicitly. |
