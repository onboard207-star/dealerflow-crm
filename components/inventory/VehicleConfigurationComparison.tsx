import type { VehicleConfigurationComparison as Comparison } from "@/lib/application/vehicle-catalog";

export function VehicleConfigurationComparison({ comparison }: { comparison: Comparison }) {
  const grouped = comparison.facts.reduce<Record<string, Comparison["facts"][number][]>>((groups, fact) => {
  ...
  return <section className="rounded-xl border bg-card p-5 shadow-soft sm:p-6" aria-labelledby="vehicle-comparison-heading">
    ...
    <div className="mt-5 space-y-6">{Object.entries(grouped).map(([category, facts]) => <div key={category}>
      <h3 className="mb-2 text-sm font-semibold">{category}</h3>
      <div className="overflow-x-auto"><table className="w-full min-w-[44rem] border-collapse text-left text-sm">
