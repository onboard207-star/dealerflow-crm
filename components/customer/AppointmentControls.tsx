...
export function AppointmentControls({ organizationId, customerId, leadId, locationId, nextAppointment, canCreate, canUpdate=false }: AppointmentControlsProps) {
  ...
  return <section aria-labelledby="appointment-controls-heading" className="rounded-xl border bg-card p-4 text-card-foreground shadow-soft sm:p-5">
    ...
    {nextAppointment&&canUpdate?<div className="mt-3 flex flex-wrap gap-2">{nextAppointment.status==="scheduled"?<Button disabled={pending} onClick={()=>transition("confirmed")} type="button" variant="outline">Confirm</Button>:null}{["scheduled","confirmed"].includes(nextAppointment.status)?<Button disabled={pending} onClick={()=>transition("arrived")} type="button" variant="outline">Mark arrived</Button>:null}{nextAppointment.status==="arrived"?<Button disabled={pending} onClick={()=>transition("completed")} type="button">Complete</Button>:null}{["scheduled","confirmed"].includes(nextAppointment.status)?<form className="flex gap-2" onSubmit={(event)=>{event.preventDefault();const form=new FormData(event.currentTarget);void transition(field(form,"outcome")as"cancelled"|"no-show",field(form,"reason"));}}><select aria-label="Missed appointment outcome" className="focus-ring h-11 rounded-lg border bg-background px-2 text-sm" name="outcome"><option value="cancelled">Cancelled</option><option value="no-show">No-show</option></select><input aria-label="Appointment outcome reason" className="focus-ring h-11 min-w-0 rounded-lg border bg-background px-3 text-sm" maxLength={1000} name="reason" placeholder="Reason" required/><Button disabled={pending} type="submit" variant="outline">Record</Button></form>:null}</div>:null}
